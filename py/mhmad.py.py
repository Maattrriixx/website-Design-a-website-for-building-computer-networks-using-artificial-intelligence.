...
`wired_network_redesign_ideas.md`:
  ...
  6. A DNS device is added to the backbone (behind the Firewall, same
     segment/VLAN as the Server).
...
VLANS = {
    'vlan_10': {
        ...
        'devices': ['Router', 'Firewall', 'Core Switch', 'Switch', 'Server', 'DNS', 'UPS'],
    },
    'vlan_20': {
        'id': 20,
        'name': 'Staff',
...
def get_vlan_for_device(device_type):
    if device_type in ('Router', 'Firewall', 'Switch', 'Core Switch', 'Server', 'DNS', 'UPS'):
        return VLANS['vlan_10']
...
def generate_devices(
    ...
    devices.append({
        'device_id': dev_id, 'type': 'Switch', 'layer': 'Core',
        'room_id': backbone_room_id, 'x': core_x, 'y': core_y, 'ports': 48,
        'notes': 'Core Switch', 'connectivity': 'wired', 'vlan': VLANS['vlan_10'],
    })
    core_sw_id = dev_id
    dev_id += 1

    # Router port sizing: capacity must cover everything directly
    # attached to the backbone segment (Core Switch, Firewall, Server,
    # DNS), per the agreed rule -- deterministic, not part of the ILP.
    backbone_member_count = 4  # Core Switch + Firewall + Server + DNS
    router_ports = size_switch_ports(backbone_member_count)

    router_x, router_y = place_central((6, 0))
    devices.append({
        'device_id': dev_id, 'type': 'Router', 'room_id': backbone_room_id,
        'x': router_x, 'y': router_y, 'ports': router_ports,
    ...
    devices.append({
        'device_id': dev_id, 'type': 'Server', 'role': 'DC/DHCP', 'room_id': backbone_room_id,
        'x': server_x, 'y': server_y,
        'notes': 'Main Server', 'connectivity': 'wired', 'vlan': VLANS['vlan_10'],
    })
    server_id = dev_id
    dev_id += 1

    # DNS: behind the Firewall, same segment/VLAN as the Server.
    dns_x, dns_y = place_central((6, 8))
    devices.append({
        'device_id': dev_id, 'type': 'DNS', 'room_id': backbone_room_id,
        'x': dns_x, 'y': dns_y,
        'notes': 'DNS Server (behind Firewall, Server segment)',
    ...
    devices.append({
        ...
        'protects': [core_sw_id, router_id, firewall_id, server_id, dns_id],
    ...
    if security_room_id is not None:
        ...
        nvr_device = {
            ...
            'vlan': VLANS['vlan_50'],
            '_nearest_switch_id': nearest_sw['device_id'] if nearest_sw else None,
        }
...
def build_connections(
    ...
    # --------------------------------------------------------
    if nvr_device is not None and nvr_device.get('_nearest_switch_id') is not None:
        sw = devices_by_id[nvr_device['_nearest_switch_id']]
    ...
    # --------------------------------------------------------
    # Core -> Router -> Firewall -> Server, Firewall -> DNS.
    # --------------------------------------------------------
    ...
    backbone_link(core_sw, router)
    backbone_link(router, firewall)
    backbone_link(firewall, server)
    backbone_link(firewall, dns)  # DNS: behind Firewall, Server's segment.
...
def run_wired_optimizer(
    ...
    (
        ...
        core_sw_id, router_id, firewall_id, server_id, dns_id, nvr_device,
    ) = generate_devices(
